...
def draw_val_images(
    *,
    ...
    ings_pred = (torch.sigmoid(output[:, ings_start_idx:]) > 0.5).cpu().numpy()
    ...
    for pred_inx, pred_key in enumerate(prediction_keys):
        ...
        truth, pred = (
            kcal_cpu.squeeze(-1).numpy(),
            output[:, pred_inx : pred_inx + 1].cpu().squeeze(-1).numpy(),
        )
...
def MyLoader(datadir: str, ds: str, batch_size: int):
    # ds = train, test, val
    dataset = FoodDataset(calories_file=datadir / f"{ds}.json", image_dir=datadir / ds,)
    return DataLoader(
...
def get_args(predict: bool):
    ...
    if not predict:
        ...
        parser.add_argument("--input-file", help="input jpg", required=True)
    parser.add_argument(
        "--train-type",
        required=True,
...
class Model:
    def __init__(self, args):
        ...
        def loss_top_ingredients(pred, data):
            ...
            if training_type == "kcal+nut+topings":
                ...
                bce = (
                    binary_cross_entropy_with_logits(
                        pred[:, ings_start_idx:], data["ingredients"]
                    )
                    * args.bce_weight
                )
                if (
        ...
        loss_fns["l1_kcal"] = lambda pred, data: nn.functional.l1_loss(
            pred, data["kcal"]
        )
        ...
        if training_type.startswith("kcal+nut"):
            ...
            self.num_output_neurons = len(prediction_keys)
    ...
    def train(self, epochs):
        ...
        train_loader = MyLoader(datadir, "train", self.batch_size)
        val_loader = MyLoader(datadir, "val", self.batch_size)
        ...
        for epoch in range(1, epochs + 1):
                        ...
                        for val_batch_inx, data in enumerate(
                            ...
                            if val_batch_inx == 0:
                                ...
                                draw_val_images(
                                    writer=writer,
                                    output=output,
                                    ings_start_idx=self.ings_start_idx,
                                    data=data,
                                    image=image,
                                    device=self.device,
            ...
            self.model.save(net, f"epoch-{epoch:03d}", logdir)

        writer.close()
        do_test = "test" in args.test
        do_train = "train" in args.test
        ...
        if do_test:
            ...
            with torch.no_grad():
                ...
                for epoch_batch_idx, data in enumerate(test_loader, 0):
                    ...
                    target_data = {
                        k: v.to(self.device) for k, v in data.items() if k != "image"
                    }
...
if __name__ == "__main__":
    with torch.autograd.detect_anomaly():
        ...
        if args.test == "test":
            epochs = 0
        Model(args).train(epochs=epochs)
